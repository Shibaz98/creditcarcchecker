...
const validateCred = arr =>{
    ...
    for(let i = arr.length -2; i >=0; i-=2){
        let oddValueAtIndex = arr[i]
        oddValueAtIndex *= 2; 
        if(oddValueAtIndex > 9){
            oddValueAtIndex -=9
        }
        newArr.push(oddValueAtIndex)
    ...
    let evenValueAtIndex = arr[i];
    newArr.push(evenValueAtIndex)
    };
    
    // sum of newArr function
...
const findInvalidCards = arr =>{
    ...
    for(let i = 0; i < batch.length; i++){
        if(validateCred(batch[i]) === false){
            invalidCardsArray.push(batch[i])
        }
